...
<div class="container mx-auto mt-10 px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($videojuegos as $videojuego)
            <div class="bg-gray-800 p-6 rounded-lg shadow-lg hover:bg-gray-700 transition duration-300 flex items-start">
                @if($videojuego->imagen)
                    <img src="{{ Storage::url($videojuego->imagen) }}" alt="{{ $videojuego->nombre }}" class="w-32 h-32 object-cover rounded mr-4">
                @else
                    <div class="w-32 h-32 bg-gray-700 rounded flex items-center justify-center text-gray-400 mr-4">
                        Sin imagen
                    </div>
                @endif
                <div class="flex-1">
